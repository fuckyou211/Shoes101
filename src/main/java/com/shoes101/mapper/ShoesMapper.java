...
//    List<Integer> getAllShoesId();
    ...
    @Select("select DISTINCT shoes.shoesid,shoes.shoesname,shoes.adddate,shoessku.price from shoes INNER JOIN shoessku on shoes.shoesid=shoessku.shoesid where shoessku.quantity > 0 and shoes.isdropoff=1 and shoes.catalogid = #{catalogId}")
    ...
    List<FGoodsVo> getFGoodsVoByPvIdPage(@Param("propertyValueId") Integer propertyValueId,@Param("start")Integer start,@Param("size")Integer size);
    //根据品牌获得相应的鞋的数量
    ...
    List<FGoodsVo> searchByNamePage(@Param("name")String name,@Param("start")Integer start,@Param("size")Integer size);
    //按名字搜索得到的鞋的数量
